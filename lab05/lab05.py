...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if label(t) == 'loki' and is_leaf(t) :
        ...
        result =[replace_loki_at_leaf(i, lokis_replacement) for i in branches(t)]
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city = make_city(get_name,lat,lon)
    citya_distance = distance(city,city_a)
    cityb_distance = distance(city,city_b)
    if citya_distance > cityb_distance:
        return get_name(city_b)
    elif citya_distance < cityb_distance:
...
def hailstone_tree(n, h):
    ...
    if n!=1 and n!=4 and (n-1)%3==0:
        branches += (hailstone_tree((n-1))//3,h-1)
    return tree(n, branches)"""
    if h == 0:
        return tree(n)
    branches = [hailstone_tree(n * 2, h - 1)]
    if n!=1 and (n - 1) % 3 == 0 and (n - 1) // 3 > 1:
        branches += [hailstone_tree((n - 1) // 3, h - 1)]
    return tree(n, branches)
